...
const defaultMenuLinks = [
  { label: "header.nav.partner", href: "#partner" },
  { label: "header.nav.documentation", href: "#documentation" },
];
            ...
            <div className="logo">
              <Link href="/">
                <div className="logo-wrapper">
                  <svg className="logo-svg" xmlns="http://www.w3.org/2000/svg" version="1.1" viewBox="0 0 666 244">
                    <defs>
                      <style>
                        {`.cls-1 { fill: #fff; }
            ...
            <div className="mobile-nav">
              <ul className="mobile-nav-links">
                ...
                <li className="mobile-language-selector">
                  <button 
                    className={`mobile-lang-toggle ${isLangMenuOpen ? 'active' : ''}`}
                    onClick={() => setIsLangMenuOpen(!isLangMenuOpen)}
                  >
                    <Globe size={20} />
                  ...
                  {isLangMenuOpen && (
                    <div className="mobile-lang-menu">
                      {languages.map((lng) => (
                        <button
                          key={lng}
                          className={`mobile-lang-option ${i18n.language === lng ? 'active' : ''}`}
                          onClick={() => changeLanguage(lng)}
                        >
                          {t(`languages.${lng}`)}
                        </button>
